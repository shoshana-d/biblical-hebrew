...
document.addEventListener('DOMContentLoaded', function() {
   ...
   var onloadExerciseTablesClass = document.getElementsByClassName("onload-exercise-table-revised");
   for (i = 0; i < onloadExerciseTablesClass.length; i++) {
      var thisSpec = onloadExerciseTablesClass[i];
      createExerciseTableRevised(thisSpec);
   }	
...
function reCreateExerciseTableRevised(thisTableId){
   ...
   createExerciseTableRevised(instructionsDiv);

}	

function createExerciseTableRevised(thisDiv){
	...
	var translations = thisDiv.getElementsByClassName("js-exercise-table-translation");
	var references = thisDiv.getElementsByClassName("js-exercise-table-reference");
	var hebrewAudio = thisDiv.getElementsByClassName("js-exercise-table-hebrew-audio"); // includes directory
	var hebrewText = thisDiv.getElementsByClassName("js-exercise-table-hebrew-text");
	var answerWords = thisDiv.getElementsByClassName("js-exercise-table-answer-words");
	var shouldKnowWords = thisDiv.getElementsByClassName("js-exercise-table-should-know-words");
	
	//var audioDir = thisDiv.getElementsByClassName("js-exercise-table-audio-dir")[0].innerHTML.trim();
	var nselectionDiv = thisDiv.getElementsByClassName("js-exercise-table-nselection");
	    ...
	    // is this hebrew word an answer word?
		  for (j=0; j < hebrewAnswerWordNumbers.length; j++) {
			 if (hebrewAnswerWordNumbers[j]-1 ==  i){
		        hebrewPara.addEventListener("click", function(){exerciseTableAnswerEventListener(event, thisTable.id);});
...
//test("hello from createExerciseTableRevised,r=" + r + ",nitems=" + nItems);   
		 ...
		 // is this hebrew word a should know word?
          ...
          if (!(isAnswerWord)) {hebrewPara.addEventListener("click", function(){exerciseTableWrongAnswerEventListener(event, thisTable.id);});}
...
//        col3.appendChild(audioElement);
	 ...
	 //   col2.appendChild(thisAudio);
	    if (anyShouldKnowWords){
		   ...
		   span4.addEventListener("click", function(){exerciseTableShowTranslationEventListener(event);});
